...
description: "Use when the user wants an adversarial Agent loop: set a task as a goal, let one subagent implement while another reviews, integration-tests, or attacks the result, then iterate until evidence shows the goal is complete. Trigger on \"开 loop\", \"对抗闭环\", \"loop engineering\", \"设置成 goal\", \"一个 subagent 干活另一个 review\", \"让另一个 agent 集成测试\", \"对抗地测试/找问题\", or \"跑完并验证\"."
---
...
3. Spawn or simulate separate roles:
   - `worker`: implement or produce the artifact.
   - `reviewer/tester`: find bugs, missing cases, integration failures, requirement drift.
...
5. Run at most 2 full adversarial rounds by default; continue only for high-risk work or clear progress.
...
Use the generated worker/reviewer/tester prompts directly when spawning subagents. If subagent tools are unavailable, run the same prompts as separate passes in the main thread.

## Prompts

Reviewer/tester stance:

```text
Find problems. Do not praise. Re-read the original goal and attack the result with real checks where possible. Report only blocking issues, important issues, missing evidence, and residual risk.
```
...
- Review findings and fix status.
